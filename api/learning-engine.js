...
===========================================================
 TradeMind Pro
 V25.6 — EDGE HEALTH FAILURE ANATOMY DIAGNOSTIC ENGINE
...
function buildV227EVPersistenceFailureAnatomy(
    ...
    */
    ...
    return result;
        ...
        // Diagnostic only.
        //
        // Uses the EXISTING V22.9 chronological transition
        // observations already produced inside this engine.
        //
        // It does NOT fetch another dataset and does NOT alter
        // discovery, qualification, validation, OOS, exits, risk,
        // thresholds, or trading.
        //
        // Only observations already classified HEALTHY from the
        // completed prior window are examined. Forward outcomes
        // are used only to split those observations into SUCCESS,
        // FAILURE or UNRESOLVED.
        //
        // V22.9 checkpoints intentionally overlap. Therefore this
        // is NOT independent confirmation and must never be treated
        // as such.
        // =====================================================
        function buildV256EdgeHealthFailureAnatomyDiagnostic(
            v229Audit
        ) {

            const checkpoints =
                safeArray(
                    v229Audit?.checkpointResults
                );
            ...
            function forwardStatus(row) {

                if (
                    row.forwardSuccess === true
                ) {
                    return "SUCCESS";
                }

                if (
                    row.forwardSuccess === false
                ) {
                    return "FAILURE";
                }
            ...
            const observations =
                healthy.map(
                    row => ({
                        ...
                        previousEV:
                            num(
                                row.context?.previousEV
                            ),

                        evMomentum:
                            num(
                                row.context?.evMomentum
                            ),
                        ...
                        priorWinRate:
                            num(
                                row.context?.priorWinRate
                            ),

                        priorSamples:
                            num(
                                row.context?.priorSamples
                            ),

                        priorDecisive:
                            num(
                                row.context?.priorDecisive
                            ),

                        internalEarlyEV:
                            num(
                                row.context?.internalEarlyEV
                            ),

                        internalLateEV:
                            num(
                                row.context?.internalLateEV
                            ),

                        internalEVChange:
                            num(
                                row.context?.internalEVChange
                            ),
            ...
            const strongestFailureAssociations =
                Object.entries(
                    featureComparison
                )
                .map(
            ...
            return {
                ...
                purpose:
                    "Explain why apparently HEALTHY prior-EV contexts succeed or fail in the forward window using frozen prior-window evidence.",
                ...
                source:
                    "V22.9_EDGE_HEALTH_PERSISTENCE_EXPANSION",
                ...
                strongestFailureAssociations:

                    strongestFailureAssociations.slice(
                        0,
                        20
                    ),
                ...
                antiLeakage: {
                    ...
                    healthStateInheritedFromPriorWindow:
                        true,

                    priorFeaturesComputedBeforeForwardOutcome:
                        true,

                    forwardOutcomeUsedOnlyForSuccessFailureLabel:
                        true,

                    futureOutcomeUsedForFeatureSelection:
                        false,

                    thresholdsOptimizedFromForwardResults:
                        false,

                    observationsAreIndependent:
                        false,

                    rollingWindowsOverlap:
                        true,

                    reasonNotIndependent:
                        "V22.9 uses overlapping chronological checkpoints by design."
                },
                ...
                decisionGuard: {
                    ...
                    interpretation:
                        "V25.6 is descriptive failure-anatomy research. It cannot authorize a trading rule or adaptive filter."
                }
            };
        }
        ...
        const v256EdgeHealthFailureAnatomyDiagnostic =
            buildV256EdgeHealthFailureAnatomyDiagnostic(
                v229EdgeHealthPersistenceExpansion
            );
        ...
        return send({
            ...
            v256EdgeHealthFailureAnatomyDiagnostic,
