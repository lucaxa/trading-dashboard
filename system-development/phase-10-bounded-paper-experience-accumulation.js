...
function main() {
  ...
  /*
   * Never append controlled smoke data to a
   * genuine LIVE_MARKET ledger.
   *
   * Controlled tests are isolated in memory
   * and written only to the result/checkpoint.
   */
  let allLedger;
  ...
  /*
   * In controlled mode the checkpoint is also
   * isolated from the persistent live position
   * state. This lets CI prove the engine works
   * without creating fake market experience.
   */
  writeJson(
    CHECK_FILE,
    result
